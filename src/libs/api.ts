...
async function openai(content: string, rootId: string): Promise<void> {
    try {
        ...
        const reader = (await openai.chat(toMessages(content))).body?.getReader();
        ...
        await logseq.Editor.insertBlock(rootId, '');
    } catch (err: any) {
        logseq.UI.showMsg(err.message, 'error');
    }
}

async function generateAdvancedQuery(content: string, rootId: string) {
    try {
